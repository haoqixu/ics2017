...
paddr_t page_translate(vaddr_t addr, bool is_write) {
  ...
  if (cpu.cr0.paging) {
    ...
    pde.val = paddr_read((intptr_t)&pgdir[(addr >> 22) & PAGE_MASK], 4);
    assert(pde.present);
    pde.accessed = 1;

    pgtab = (PTE *)(intptr_t)(pde.page_frame << 12);
    pte.val = paddr_read((intptr_t)&pgtab[(addr >> 12) & PAGE_MASK], 4);
    assert(pte.present);
    pte.accessed = 1;
...
uint32_t vaddr_read(vaddr_t addr, int len) {
  ...
  Log("vaddr_read: 0x%x len:%d", addr, len);
...
void vaddr_write(vaddr_t addr, int len, uint32_t data) {
  ...
  Log("vaddr_write: 0x%x len:%d", addr, len);
